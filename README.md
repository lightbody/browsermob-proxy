...
Shuts down the proxy and closes the port. *[port]* in request path it is port where your proxy was started | DELETE | */proxy/[port]* |*graceful* - Boolean. False, Doesn't wait for pending requests to finish before closing the proxy. Optional, default to "true"|
